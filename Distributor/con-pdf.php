...
<body>
	...
	$pdf->cell(170,15,'Consumer managed by '.$email,0,1,'C',false);
	...
	$sql=mysqli_query($conn,"select * from consumer where dis_email='$email'");
	if (mysqli_num_rows($sql)>0) {
		...
		while ($row=mysqli_fetch_array($sql)) {
			...
			$pdf->cell(20,6,$row['reg_date'],1,0);
			$pdf->cell(30,6,$row['reg_time'],1,1);
